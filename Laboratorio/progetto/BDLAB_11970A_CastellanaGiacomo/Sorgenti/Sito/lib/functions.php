...
*/
function svuotaMagazzino($conf,$cdf){
  $db=open_pg_connection();
  ...
  if($a=='00000'){
    if($conf=="CONFERMO"){
      ...
      $msg="IMPOSSIBILE SVUOTARE IL MAGAZZINO - comando errato";
    }
  }else{
    $msg="IMPOSSIBILE SVUOTARE IL MAGAZZINO - non admin";
  }
  return $msg;
}
/*
...
function mostraInfoLibroTitolo($titolo){
  ...
  $sql = "SELECT * FROM biblioteca.libro WHERE LOWER(titolo) LIKE ('%'||LOWER($1)||'%')";
  $params = array(
    $titolo
  );
