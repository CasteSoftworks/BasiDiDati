...
DROP VIEW IF EXISTS biblioteca.statistichesedi;
...
--
-- Name: statistichesedi; Type: VIEW; Schema: biblioteca; Owner: giacomo_castellana
--

CREATE VIEW biblioteca.statistichesedi AS
 SELECT copia.dove,
    count(DISTINCT copia.id) AS qid,
    count(DISTINCT copia.libro) AS qis,
    count(copia.*) FILTER (WHERE (NOT copia.disp)) AS qpr
   FROM biblioteca.copia
  GROUP BY copia.dove;


ALTER TABLE biblioteca.statistichesedi OWNER TO giacomo_castellana;
